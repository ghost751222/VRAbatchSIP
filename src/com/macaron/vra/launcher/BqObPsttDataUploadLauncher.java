...
import com.macaron.vra.processor.BqObPsttDataUploadProcessor;
import com.macaron.vra.service.impl.BqObPsttDataUploadServiceImpl;
import com.macaron.vra.util.DateUtil;

@Component
public class BqObPsttDataUploadLauncher {

	private static final Logger logger = LoggerFactory.getLogger(BqObPsttDataUploadLauncher.class);

	private static String BQ_OB_TASK_SCHEDULE_CRON_EX = "0 */15 * * * ?";
//	private static String BQ_OB_TASK_SCHEDULE_CRON_EX = "5 * * * * ?";
	
	@Autowired
	private TaskScheduler taskScheduler;
	@Autowired
	private BqObPsttDataUploadProcessor processor;
...
//				logger.info("schedule exec end ");
		...
		taskScheduler.schedule(hourTask, new CronTrigger(BQ_OB_TASK_SCHEDULE_CRON_EX));
		logger.info("schedule CRON:" + BQ_OB_TASK_SCHEDULE_CRON_EX);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			BqObPsttDataUploadLauncher launcher = (BqObPsttDataUploadLauncher)context.getBean(BqObPsttDataUploadLauncher.class);
			
			LocalDate today = LocalDate.now();
//			LocalDate today = LocalDate.of(2018, 2, 3);
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern(BqObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
...
//				String startDtStr = "2018-05-29-00-00";
//				String endDtStr = "2018-05-29-00-15";
				dtf = DateTimeFormatter.ofPattern(BqObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
				LocalDateTime now = LocalDateTime.now();
