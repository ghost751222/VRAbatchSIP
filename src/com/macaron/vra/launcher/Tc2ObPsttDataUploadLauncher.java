...
import com.macaron.vra.processor.Tc2ObPsttDataUploadProcessor;
import com.macaron.vra.service.impl.Tc2ObPsttDataUploadServiceImpl;
import com.macaron.vra.util.DateUtil;

@Component
public class Tc2ObPsttDataUploadLauncher {

	private static final Logger logger = LoggerFactory.getLogger(Tc2ObPsttDataUploadLauncher.class);

	private static String TC2_OB_TASK_SCHEDULE_CRON_EX = "0 */15 * * * ?";
//	private static String TC2_OB_TASK_SCHEDULE_CRON_EX = "5 * * * * ?";
	
	@Autowired
	private TaskScheduler taskScheduler;
	@Autowired
	private Tc2ObPsttDataUploadProcessor processor;
...
//				logger.info("schedule exec end ");
		...
		taskScheduler.schedule(hourTask, new CronTrigger(TC2_OB_TASK_SCHEDULE_CRON_EX));
		logger.info("schedule CRON:" + TC2_OB_TASK_SCHEDULE_CRON_EX);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Tc2ObPsttDataUploadLauncher launcher = (Tc2ObPsttDataUploadLauncher)context.getBean(Tc2ObPsttDataUploadLauncher.class);
			
			LocalDate today = LocalDate.now();
//			LocalDate today = LocalDate.of(2018, 2, 3);
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern(Tc2ObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
...
//				String endDtStr = "2018-05-29-00-15";
				dtf = DateTimeFormatter.ofPattern(Tc2ObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
				LocalDateTime now = LocalDateTime.now();
