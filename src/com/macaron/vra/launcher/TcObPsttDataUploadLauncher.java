...
import com.macaron.vra.processor.TcObPsttDataUploadProcessor;
import com.macaron.vra.service.impl.TcObPsttDataUploadServiceImpl;
import com.macaron.vra.util.DateUtil;

@Component
public class TcObPsttDataUploadLauncher {

	private static final Logger logger = LoggerFactory.getLogger(TcObPsttDataUploadLauncher.class);

	private static String TC_OB_TASK_SCHEDULE_CRON_EX = "0 */15 * * * ?";
//	private static String TC_OB_TASK_SCHEDULE_CRON_EX = "0 0 * * * ?";
	
	@Autowired
	private TaskScheduler taskScheduler;
	@Autowired
	private TcObPsttDataUploadProcessor processor;
...
//				logger.info("schedule exec end ");
		...
		taskScheduler.schedule(hourTask, new CronTrigger(TC_OB_TASK_SCHEDULE_CRON_EX));
		logger.info("schedule CRON:" + TC_OB_TASK_SCHEDULE_CRON_EX);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			TcObPsttDataUploadLauncher launcher = (TcObPsttDataUploadLauncher)context.getBean(TcObPsttDataUploadLauncher.class);
			
			LocalDate today = LocalDate.now();
//			LocalDate today = LocalDate.of(2018, 2, 3);
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern(TcObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
...
//			mode = "m";
			if("a".equalsIgnoreCase(mode)){
				...
				dtf = DateTimeFormatter.ofPattern(TcObPsttDataUploadServiceImpl.PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN);
				LocalDateTime now = LocalDateTime.now();
				LocalDateTime startDt = LocalDateTime.parse(startDtStr, dtf).truncatedTo(ChronoUnit.HOURS);
				LocalDateTime endDt = LocalDateTime.parse(endDtStr, dtf).truncatedTo(ChronoUnit.HOURS);
				while(startDt.isBefore(endDt)){
					...
					Date e = DateUtil.toDate(startDt.plusHours(1));
