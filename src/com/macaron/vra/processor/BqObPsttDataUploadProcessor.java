...
@Component
public class BqObPsttDataUploadProcessor {

	private static final Logger logger = LoggerFactory.getLogger(BqObPsttDataUploadProcessor.class);
	...
	@Autowired
	private BqObPsttDataUploadServiceImpl psttDataUploadService;
	...
	public Future<Future<?>> processTask(Date dataTimeStart, Date dataTimeEnd){
		...
		Future<?> ret = taskExecutors.submit(new BqObPsttDataUploadTask(dataTimeStart, dataTimeEnd));
		logger.info("put task in taskExecPool end");
		return new AsyncResult<Future<?>>(ret);
	}
	
	private class BqObPsttDataUploadTask implements Runnable {
		private Date dataTimeStart;
		private Date dataTimeEnd;

		public BqObPsttDataUploadTask(Date dataTimeStart, Date dataTimeEnd) {
			super();
