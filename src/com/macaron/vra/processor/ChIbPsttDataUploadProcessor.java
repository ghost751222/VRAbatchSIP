...
import com.macaron.vra.service.impl.ChIbPsttDataUploadServiceImpl;

@Component
public class ChIbPsttDataUploadProcessor {

	private static final Logger logger = LoggerFactory.getLogger(ChIbPsttDataUploadProcessor.class);
	...
	@Autowired
	private ChIbPsttDataUploadServiceImpl psttDataUploadService;
	...
	public Future<Future<?>> processTask(Date dataTimeStart, Date dataTimeEnd){
		...
		Future<?> ret = taskExecutors.submit(new ChIbPsttDataUploadTask(dataTimeStart, dataTimeEnd));
		logger.info("put task in taskExecPool end");
		return new AsyncResult<Future<?>>(ret);
	}
	
	private class ChIbPsttDataUploadTask implements Runnable {
		private Date dataTimeStart;
		private Date dataTimeEnd;

		public ChIbPsttDataUploadTask(Date dataTimeStart, Date dataTimeEnd) {
			super();
