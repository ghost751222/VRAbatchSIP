...
import com.macaron.vra.vo.JbObCallResultRecordInfoVo;
import com.macaron.vra.vo.JbObProcessVo;

@Service
public class JbObPsttDataUploadServiceImpl {

	private static final Logger logger = LoggerFactory.getLogger(JbObPsttDataUploadServiceImpl.class);
	
	private static final String PSTT_DATA_SUFFIX_JB = "-jb";
	private static final String FUGO_API_LOCATION = "1004";
	private static final String BASE_RECORDING_DIR_PATH = "\\\\jb-recnas\\RecordFiles/Recording";
	...
	public void process(Date dataTimeStart, Date dataTimeEnd){
		...
		try {
			logger.info("JbObPsttDataUpload start");
...
//			}
			...
			String batchSerial = DateUtil.getNowFullAd() + PSTT_DATA_SUFFIX_JB;
			logger.info("batchSerial:{}", batchSerial);
			
			String psttTaskSerial = DateTimeFormatter.ofPattern(PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN).format(ldtStart) + PSTT_DATA_SUFFIX_JB;
			...
			String psttFtpDataSourceDirPath = AppConst.jbObPsttFtpDataSourceDirPath;
			...
			if(obj.get("rtncode").getAsInt()==1){
				...
				Type listType = new TypeToken<List<JbObCallResultRecordInfoVo>>() {}.getType();
				List<JbObCallResultRecordInfoVo> resultRecordInfoVos = new Gson().fromJson(dataArray, listType);

				/*
				 * 拉音檔，轉檔，放置對應目錄
				 */
				Map<String, JbObProcessVo> processVoMap = combineDataAndCvtWav(resultRecordInfoVos, speechTaskRecsDirPath, cvtWavTmpDirPath);
				logger.info("processVoMap size:{}", processVoMap.entrySet().size());
				for (Entry<String, JbObProcessVo> e : processVoMap.entrySet()) {
				 ...
				 * 上傳音檔及DIM檔
				 */
				String username = AppConst.jbObPsttFtpUsername;
				logger.info("{}", username);
				String password = AppConst.jbObPsttFtpPassword;
				logger.info("{}", password);
				String host = AppConst.jbObPsttFtpHost;
				logger.info("{}", host);
				Integer port = AppConst.jbObPsttFtpPort;
				logger.info("{}", port);
				String dataSourceDirPath = AppConst.jbObPsttFtpDataSourceDirPath;
...
//				upload Speech
				...
				logger.info("JbObPsttDataUpload end");
	...
	private Map<String, JbObProcessVo> combineDataAndCvtWav(
			List<JbObCallResultRecordInfoVo> resultRecordInfoVos,
			String speechTaskRecsDirPath,
			String cvtWavWorkDirPath){
		logger.info("start grouping fugoData ");
		Map<String, List<JbObCallResultRecordInfoVo>> groupMap = new HashMap<String, List<JbObCallResultRecordInfoVo>>();
		for (JbObCallResultRecordInfoVo infoVo : resultRecordInfoVos) {
			String connId = infoVo.getConnid();
			List<JbObCallResultRecordInfoVo> list = groupMap.get(connId);
			if(list==null){
				list = new ArrayList<JbObCallResultRecordInfoVo>();
				list.add(infoVo);
		...
		Map<String, JbObProcessVo> processVoMap = new HashMap<String, JbObProcessVo>();
		logger.info("start combine by grouping fugoData ");
		for (Entry<String, List<JbObCallResultRecordInfoVo>> entry : groupMap.entrySet()) {
			String connId = entry.getKey();
			List<JbObCallResultRecordInfoVo> list = entry.getValue();
			JbObProcessVo procVo = new JbObProcessVo();
			if(list!=null && list.size()>0){
				try {
					for (int i=0; i<list.size(); i++) {
						JbObCallResultRecordInfoVo vo = list.get(i);
						if(i==0){
						...
						if(i>0){
							...
							JbObCallResultRecordInfoVo cbnVo = procVo.getResultRecordInfoVo();
...
//							logger.info("process connId:{}",procVo.getResultRecordInfoVo().getConnid());
					...
					if(!wavExists){
						...
						//cvt JB wav
						WavUtil.cvtJbWav(procVo.getSrcWavPath(), speechTaskRecsDirPath, cvtWavWorkDirPath);
					}
	...
	private void settingProcessData(JbObProcessVo processVo, String speechTaskRecsDirPath){
		JbObCallResultRecordInfoVo infoVo = processVo.getResultRecordInfoVo();
...
//		String subFilePath = fileName.replaceAll("Recording/", "");
	...
	private void writeDim(
			Map<String, JbObProcessVo> processVoMap, 
...
//		int c = 0;
		...
		try {
			...
			for(Entry<String, JbObProcessVo> entry : processVoMap.entrySet()){
				JbObProcessVo processVo = entry.getValue();
				//ex:3241@@#.wav
				String baseFileName = processVo.getBaseFileName();
				JbObCallResultRecordInfoVo vo = processVo.getResultRecordInfoVo();
				//speech
	...
	private boolean exists(String destFilePath){
		try {
			return FtpUtil.exists(
					AppConst.jbObPsttFtpUsername, 
					AppConst.jbObPsttFtpPassword, 
					AppConst.jbObPsttFtpHost, 
					AppConst.jbObPsttFtpPort,  
					destFilePath);
		} catch (Exception e) {
			logger.error("{}", e);
