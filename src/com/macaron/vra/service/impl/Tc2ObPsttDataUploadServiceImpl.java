...
import com.macaron.vra.vo.Tc2ObCallResultRecordInfoVo;
import com.macaron.vra.vo.Tc2ObProcessVo;

@Service
public class Tc2ObPsttDataUploadServiceImpl {

	private static final Logger logger = LoggerFactory.getLogger(Tc2ObPsttDataUploadServiceImpl.class);
	
	private static final String PSTT_DATA_SUFFIX_TC2 = "-tc2";
	private static final String FUGO_API_LOCATION = "1007";
	private static final String BASE_RECORDING_DIR_PATH = "\\\\tc2-recnas\\RecordFiles/Recording/";
	...
	public void process(Date dataTimeStart, Date dataTimeEnd){
		...
		try {
			logger.info("Tc2PsttDataUpload start");
...
//			}
			...
			String batchSerial = DateUtil.getNowFullAd() + PSTT_DATA_SUFFIX_TC2;
			logger.info("batchSerial:{}", batchSerial);
			
			String psttTaskSerial = DateTimeFormatter.ofPattern(PSTT_TASK_SERIAL_PREFIX_DATE_PATTERN).format(ldtStart) + PSTT_DATA_SUFFIX_TC2;
			...
			String psttFtpDataSourceDirPath = AppConst.tc2ObPsttFtpDataSourceDirPath;
			...
			if(obj.get("rtncode").getAsInt()==1){
				...
				Type listType = new TypeToken<List<Tc2ObCallResultRecordInfoVo>>() {}.getType();
				List<Tc2ObCallResultRecordInfoVo> resultRecordInfoVos = new Gson().fromJson(dataArray, listType);

				/*
				 * 拉音檔，轉檔，放置對應目錄
				 */
				Map<String, Tc2ObProcessVo> processVoMap = combineDataAndCvtWav(resultRecordInfoVos, speechTaskRecsDirPath, cvtWavTmpDirPath);
				logger.info("processVoMap size:{}", processVoMap.entrySet().size());
				for (Entry<String, Tc2ObProcessVo> e : processVoMap.entrySet()) {
				 ...
				 * 上傳音檔及DIM檔
				 */
				String username = AppConst.tc2ObPsttFtpUsername;
				logger.info("{}", username);
				String password = AppConst.tc2ObPsttFtpPassword;
				logger.info("{}", password);
				String host = AppConst.tc2ObPsttFtpHost;
				logger.info("{}", host);
				Integer port = AppConst.tc2ObPsttFtpPort;
				logger.info("{}", port);
				String dataSourceDirPath = AppConst.tc2ObPsttFtpDataSourceDirPath;
...
//				upload Speech
				...
				logger.info("Tc2ObPsttDataUpload end");
	...
	private Map<String, Tc2ObProcessVo> combineDataAndCvtWav(
			List<Tc2ObCallResultRecordInfoVo> resultRecordInfoVos,
			String speechTaskRecsDirPath,
			String cvtWavWorkDirPath){
		logger.info("start grouping fugoData ");
		Map<String, List<Tc2ObCallResultRecordInfoVo>> groupMap = new HashMap<String, List<Tc2ObCallResultRecordInfoVo>>();
		for (Tc2ObCallResultRecordInfoVo infoVo : resultRecordInfoVos) {
			String connId = infoVo.getConnid();
			List<Tc2ObCallResultRecordInfoVo> list = groupMap.get(connId);
			if(list==null){
				list = new ArrayList<Tc2ObCallResultRecordInfoVo>();
				list.add(infoVo);
		...
		Map<String, Tc2ObProcessVo> processVoMap = new HashMap<String, Tc2ObProcessVo>();
		logger.info("start combine by grouping fugoData ");
		for (Entry<String, List<Tc2ObCallResultRecordInfoVo>> entry : groupMap.entrySet()) {
			String connId = entry.getKey();
			List<Tc2ObCallResultRecordInfoVo> list = entry.getValue();
			Tc2ObProcessVo procVo = new Tc2ObProcessVo();
			if(list!=null && list.size()>0){
				try {
					for (int i=0; i<list.size(); i++) {
						Tc2ObCallResultRecordInfoVo vo = list.get(i);
						if(i==0){
						...
						if(i>0){
							...
							Tc2ObCallResultRecordInfoVo cbnVo = procVo.getResultRecordInfoVo();
...
//							}
					...
					if(!wavExists){
						...
						//cvt Tc2 wav
						WavUtil.cvtTc2Wav(procVo.getSrcWavPath(), speechTaskRecsDirPath, cvtWavWorkDirPath);
					}
	...
	private void settingProcessData(Tc2ObProcessVo processVo, String speechTaskRecsDirPath){
		Tc2ObCallResultRecordInfoVo infoVo = processVo.getResultRecordInfoVo();
...
//		String subFilePath = fileName.replaceAll("Recording/", "");
	...
	private void writeDim(
			Map<String, Tc2ObProcessVo> processVoMap, 
...
//		int c = 0;
		...
		try {
			...
			for(Entry<String, Tc2ObProcessVo> entry : processVoMap.entrySet()){
				Tc2ObProcessVo processVo = entry.getValue();
				//ex:3241@@#.wav
				String baseFileName = processVo.getBaseFileName();
				Tc2ObCallResultRecordInfoVo vo = processVo.getResultRecordInfoVo();
				//speech
	...
	private boolean exists(String destFilePath){
		try {
			return FtpUtil.exists(
					AppConst.tc2ObPsttFtpUsername, 
					AppConst.tc2ObPsttFtpPassword, 
					AppConst.tc2ObPsttFtpHost, 
					AppConst.tc2ObPsttFtpPort,  
					destFilePath);
		} catch (Exception e) {
			logger.error("{}", e);
